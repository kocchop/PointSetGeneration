...
def showpoints(xyz,c0=None,c1=None,c2=None,waittime=0,showrot=False,magnifyBlue=0,freezerot=False,background=(0,0,0),normalizecolor=True,ballradius=10):
    ...
    def render():
        ...
        print "xanlge: {}, yangle: {}".format(xangle,yangle)
        ...
        if magnifyBlue>0:
            ...
            if magnifyBlue>=2:
                show[:,:,0]=np.maximum(show[:,:,0],np.roll(show[:,:,0],-1,axis=1))
        if showrot:
            cv2.putText(show,'xangle %d'%(int(xangle/np.pi*180)),(30,showsz-30),0,1.0,(255,0,0),1)
            cv2.putText(show,'yangle %d'%(int(yangle/np.pi*180)),(30,showsz-50),0,1.0,(255,0,0),1)
            cv2.putText(show,'zoom %d%%'%(int(zoom*100)),(30,showsz-70),0,1.0,(255,0,0),1)
    changed=True
    while True:
