...
def render(xyz,mousex,mousey,c0,c1,c2,showrot,magnifyBlue,freezerot,background,normalizecolor,ballradius):
    ...
    if magnifyBlue>0:
        ...
        if magnifyBlue>=2:
            show[:,:,0]=np.maximum(show[:,:,0],np.roll(show[:,:,0],-1,axis=1))
    if showrot:
        cv2.putText(show,'xangle %d'%(int(xangle/np.pi*180)),(30,showsz-30),0,1.5,(255,255,255),1)
        cv2.putText(show,'yangle %d'%(int(yangle/np.pi*180)),(30,showsz-70),0,1.5,(255,255,255),1)
        cv2.putText(show,'zoom %d%%'%(int(zoom*100)),(30,showsz-110),0,1.5,(255,255,255),1)
...
def showpoints(xyz,c0=None,c1=None,c2=None,waittime=0,showrot=False,magnifyBlue=0,freezerot=False,background=(0,0,0),normalizecolor=True,ballradius=10):
    ...
    angles = np.linspace(1,-1,101)
    
    for x in angles:
        # print x
        show = render(xyz, 0.55, x, c0,c1,c2,showrot,magnifyBlue,freezerot,background,normalizecolor,ballradius)
        frames.append(show)
        
        
        
        # show = cv2.applyColorMap(show, cv2.COLORMAP_VIR)
        # cv2.imshow('show3d',show)
        
    return showsz, frames
